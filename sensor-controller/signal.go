...
func (soc *sOperationCtx) processSignal(signal v1alpha1.Signal) (*v1alpha1.NodeStatus, error) {
	...
	if node.Phase == v1alpha1.NodePhaseNew {
		...
		if !soc.signalIsPresent(node.ID) {
			...
			// this can happen if the sensor-controller or a signal pod serving the stream goes down.
...
// stop the signal by:
// 1. deleting the stream from the sensor-controller's signalStreams map
// 2. sending the terminate signal on the stream and close it
// NOTE: this is a method on the sensor-controller
